...
class CustomAdapterForRecyclerView(var dataSet: List<Result>?) : RecyclerView.Adapter<CustomAdapterForRecyclerView.ViewHolder>() {
    ...
    */
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        val dataRecord =
            dataSet?.get(position) // to improve on performance, the row is temporarily saved.

        if (dataRecord != null) {
            val (initials, name) = getNameAndInitialsToShow(
                dataRecord.name.first,
                dataRecord.name.last
            )
            viewHolder.initialsDiscTextView.text = initials
            viewHolder.nameTextView.text = name

            viewHolder.dateOfBirthTextView.text =
                convertISO8601DateTimeStringToHumanReadableUTCDateTimeString(dataRecord.dob.date)
        }
    }
